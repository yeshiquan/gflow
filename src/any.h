...
public:
    ...
    Any(T&& value) :
        ...
        _holder(new InstanceHolder<typename ::std::decay_t<T>>(::std::forward<T>(value))),
    ...
    template<typename T>
    Any(std::unique_ptr<T>&& value) :
        _empty(false),
        _type(Type::INSTANCE),
        _instanse_type(&StaticTypeId<typename ::std::decay_t<T>>::TYPE_NAME),
        _holder(new InstanceHolder<typename ::std::decay_t<T>>(std::move(value))),
        _pointer(_holder->get()),
        _primitive_value(),
        _const_ref(false) {
        }        
...
private:
    ...
    public:
        InstanceHolder(std::unique_ptr<T>&& p) : _p_ins(std::move(p)) {}
        InstanceHolder(const T& instance) : _p_ins(std::make_unique<T>(instance)) {}
        InstanceHolder(T&& instance) : _p_ins(std::make_unique<T>(std::move(instance))) {}

        virtual void* get() {
            return _p_ins.get();
        }

        virtual InstanceHolder* clone() const {
            if constexpr (std::is_copy_constructible_v<T>) {
                return new InstanceHolder(*_p_ins);
            }
            LOG(FATAL) << "try copy a non-copyable object T=[" << type_name<T>() << "]";
            //throw std::runtime_error("try copy a non-copyable object T=[" + std::string(type_name<T>()) + "]");
            return nullptr;
        }

    private:
        std::unique_ptr<T> _p_ins;
    };
