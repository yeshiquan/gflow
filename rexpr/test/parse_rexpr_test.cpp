...
auto parse = [](std::string const& source, fs::path input_path)-> std::string {
    ...
    using rexpr::parser::IteratorType;
    IteratorType iter(source.begin());
    IteratorType const end(source.end());
    ...
    using rexpr::parser::ErrorHandlerType;
    using rexpr::parser::error_handler_tag;
    ErrorHandlerType error_handler(iter, end, out, input_path.string()); // Our error handler
    ...
    if (success) {
        if (iter != end) {
            ...
            rexpr::ast::RexprPrinter printer(out);
            printer(ast);
        }
    }
