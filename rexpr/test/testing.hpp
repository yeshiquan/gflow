...
namespace boost::spirit::x3::testing {
...
struct CompareResult {
    CompareResult(Iterator pos, bool full_match) : 
...
CompareResult<typename Range::const_iterator>
...
////////////////////////////////////////////////////////////////////////////
template <typename F>
bool compare(fs::path input_path, fs::path expect_path, F f, char const* re_prefix = "<%", char const* re_suffix = "%>");
...
template <typename Iterator>
inline bool is_regex(Iterator& first, Iterator last, std::string& re, char const* re_prefix, char const* re_suffix) {
    boost::regex e(re_prefix + std::string("(.*?)") + re_suffix);
    boost::match_results<Iterator> what;
    if (boost::regex_search(first, last, what, e, boost::match_default | boost::match_continuous)) {
        re = what[1].str();
        first = what[0].second;
        return true;
    }
...
template <typename Range>
inline CompareResult<typename Range::const_iterator>
compare(Range const& in, Range const& tem, char const* re_prefix, char const* re_suffix) {
    using iter_t = typename Range::const_iterator;
    using CompareResult_t = CompareResult<iter_t> ;
    ...
    while (in_first != in_last && tem_first != tem_last) {
        if (is_regex(tem_first, tem_last, re, re_prefix, re_suffix)) {
            ...
            boost::regex e(re);
            if (!boost::regex_search(in_first, in_last, what, e, boost::match_default | boost::match_continuous)) {
                // RE mismatch: exit now.
                return CompareResult_t(in_first, false);
            } else {
    ...
    return CompareResult_t(in_first, !has_trailing_nonspaces);
}
...
template <typename F>
inline bool compare(fs::path input_path, fs::path expect_path, F f, char const* re_prefix, char const* re_suffix) {
