...
//#include <baidu/feed/mlarch/babylon/concurrent/transient_queue.h>
...
struct ObjNonCopy {
    ...
    ObjNonCopy(const ObjNonCopy&) = delete;
    int id = 0;
};

TEST_F(AnyTest, test_obj_non_copy) {
    /*
    auto p = std::make_unique<ObjNonCopy>();
    p->id = 10;
    baidu::feed::mlarch::babylon::Any any1(std::move(p));
    ObjNonCopy* obj = any1.get<ObjNonCopy>();
    ...
    */

    {
        ...
        p->id = 10;
        Any any1(std::move(p));
        ASSERT_EQ(10, any1.get<ObjNonCopy>()->id);
        Any any2(any1);
        //ASSERT_EQ(nullptr, any2.get<ObjNonCopy>());
    }
    ASSERT_EQ(0, counter);
    ...
    ObjNonCopy obj2(obj);
    Any any1(obj);
    ASSERT_EQ(obj.id, any1.get<ObjNonCopy>()->id);

    std::vector<std::string> items;
    Any any2(items);

    Queue<ObjNonCopy> queue;
    //auto* q2 = new Queue<ObjNonCopy>(queue);
    if constexpr (std::is_copy_constructible_v<Queue<ObjNonCopy>>) {
        std::cout << "is_copy_constructible:true\n";
    } else {
        std::cout << "is_copy_constructible:false\n";
    }

    baidu::feed::mlarch::babylon::Any any5(queue);
    //Any any3(std::move(queue));
    */
}
